...
const PORT_FILE = process.env.GSDLEARN_PORT_FILE || null;
...
function preferredPort() {
  if (process.env.GSDLEARN_PORT) return Number(process.env.GSDLEARN_PORT);
  if (PORT_FILE) {
    try {
...
const HOST = process.env.GSDLEARN_HOST || '127.0.0.1';
const URL_HOST = process.env.GSDLEARN_URL_HOST || (HOST === '127.0.0.1' ? 'localhost' : HOST);
const SESSION_DIR = process.env.GSDLEARN_DIR || '/tmp/gsdLearn';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const GSDLEARN_VERSION = readGsdLearnVersion();
const GSDLEARN_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/gsd-visual-learning-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'GSDLEARN_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const GSDLEARN_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
let ownerPid = process.env.GSDLEARN_OWNER_PID ? Number(process.env.GSDLEARN_OWNER_PID) : null;
...
// Persisted alongside the port (GSDLEARN_TOKEN_FILE) so a restart keeps the
// same key and an already-open tab's cookie still validates.
const TOKEN_FILE = process.env.GSDLEARN_TOKEN_FILE || null;
...
function initialToken() {
  if (process.env.GSDLEARN_TOKEN) {
    return { value: process.env.GSDLEARN_TOKEN, source: 'env' };
  }
  if (TOKEN_FILE) {
    try {
...
let COOKIE_NAME = 'gsdLearn-key-' + PORT; // refined to the actual bound port in onListen
...
<html>
<head><meta charset="utf-8"><title>GSD Learn Companion</title>
<style>
...
<body><!-- BRANDING --><h1>GSD Learn Companion</h1>
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Opening GSD Learn Companion</title></head>
<body>
<script>
try { sessionStorage.setItem('gsdLearn-session-key', ${jsonKey}); } catch (e) {}
location.replace('/');
</script>
</body>
</html>`;
...
function brandMarkup() {
  const version = escapeHtmlText(GSDLEARN_VERSION);
  const text = GSDLEARN_TELEMETRY_DISABLED
    ? 'Prime Radiant GsdLearn v' + version
    : 'GsdLearn v' + version;
  const logo = GSDLEARN_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + GSDLEARN_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(GSDLEARN_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/obra/gsd">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
...
// browser is already connected. Override the launcher with GSDLEARN_OPEN_CMD.
let browserOpened = false;
function maybeOpenBrowser() {
  if (browserOpened) return;
  browserOpened = true;
  if (!process.env.GSDLEARN_OPEN) return; // opt-in: only after the user approves the companion
  if (HOST !== '127.0.0.1' && HOST !== 'localhost') return;
  if (clients.size > 0) return; // the user already opened it
  const url = companionUrl(); // must carry the key or the gate 403s it
  const cp = require('child_process');
  // Operator-provided launcher: run as given (this env var is trusted operator input).
  if (process.env.GSDLEARN_OPEN_CMD) {
    try { cp.exec(process.env.GSDLEARN_OPEN_CMD + ' ' + JSON.stringify(url), () => {}); } catch (e) { /* best effort */ }
    return;
  }
...
// override with GSDLEARN_IDLE_TIMEOUT_MS (start-server.sh: --idle-timeout-minutes).
const IDLE_TIMEOUT_MS = (() => {
  const ms = Number(process.env.GSDLEARN_IDLE_TIMEOUT_MS);
...
const LIFECYCLE_CHECK_MS = (() => {
  const ms = Number(process.env.GSDLEARN_LIFECYCLE_CHECK_MS);
...
function startServer() {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' && !triedFallback) {
      if (tokenSource === 'env') {
        console.error('Server failed to bind: preferred port is in use and GSDLEARN_TOKEN is set; refusing fallback with explicit token');
        process.exit(1);
      }
      triedFallback = true;
      PORT = randomPort();
