#!/usr/bin/env bash
# Stop the gsdLearn server and clean up
# Usage: stop-server.sh <session_dir>
#
# Kills the server process. Only deletes session directory if it's
# under /tmp (ephemeral). Persistent directories (.gsd/) are
...
command_has_server_id() {
  local pid="$1"
  local expected="$2"
  local expected_arg="--gsdLearn-server-id=$expected"
  if [[ -r "/proc/$pid/cmdline" ]]; then
    local arg
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if ! is_gsdLearn_server "$pid"; then
