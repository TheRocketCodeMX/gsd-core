...
**Source-of-truth precedence (resolve conflicts by rule, not oscillation):** the **locked design wins on UX/scope/structure**; the **canonical spec** (role bible / domain spec) **wins on domain facts** (roles, entities); the **old code is the authority on what was actually built and on hidden behavior** (so it wins on "is this a real requirement?") — but **never on quality/structure**. State this up front so every conflict resolves without a you-decide interruption.
...
**Salvage = backend LOGIC adapted onto the new clean model — never the old schema/tables.** The new schema is designed fresh from `DOMAIN-MODEL.md`; old-table artifacts have no place in it.
