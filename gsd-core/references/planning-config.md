...
| `workflow.certification` | enum | `"required"` | `"required"`, `"offer"`, `"off"` | **Capability-owned** — declared by `capabilities/strategy/capability.json`, not by the central schema (same federated surface as `workflow.ui_review`); listed here for discoverability. Certification posture for the pre-UAT certification step in `verify-work` (`gsd-core/references/certification.md`). `"required"` (default): every phase resolves a certification outcome before UAT — certified by the probed driver, `certification: human (CERT-0)` when no driver is capable (today's human UAT satisfies it), or a recorded `certification: N/A — no user-facing change`. A skip is always **recorded**, never silent. `"offer"`: the step asks before running. `"off"`: the step is skipped, and `verify-work` behaves exactly as it did before certification existed. The tier, probe results, and mechanism come from `.planning/TEST-STRATEGY.md` `## Certification` — this key controls only whether the loop acts on them. |
