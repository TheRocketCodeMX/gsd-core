...
**Design-provided mode (a provided design is a source of truth).** When `query project mode` reports a provided design (`has_provided_design: true`), **ingest the design BEFORE drafting the glossary** — do not model the domain from the brief/requirements alone while a design sits unread (that is the laundering that turns one literal `address` input into a generic "address" and lets a later builder invent street/city/state/zip). Read `@~/.claude/gsd-core/references/design-ingestion.md` and ingest the design from the single `## Mode` Design-input pointer:
- **Lift the literal entity/field vocabulary** — the nouns the design names and the **observable data shape** it collects (a form with one address input → one `address` field, captured as the design shows it; never generalize it away). These domain facts seed Step 3's glossary and Step 4's areas.
- **Reconcile against requirements** (KEEP / REFINE / CONTESTED), exactly like the brownfield reconciliation — and never drop a field the requirements need just because the design under-showed it.
- **Write the field oracle.** Record the design's user-facing fields (per surface, each tagged `source: design | requirement | internal`) + `covered_surfaces` into `.planning/DESIGN-INVENTORY.md` from `@~/.claude/gsd-core/templates/design-inventory.md`. This is the in-repo oracle the build loop and the design-fidelity gate read — populate it whenever the design bears a data shape (i.e. anything beyond pure styling/tokens). (First-writer-wins: if `.planning/DESIGN-INVENTORY.md` already exists — e.g. `frontend-architecture` ran first on a design-pulled-earlier path — read + reconcile against it rather than overwriting it.)
- **Strategic boundary:** take the design's **domain vocabulary and entity/field shapes** (domain facts) — *not* its screen layout, component tree, or visual styling (that is `ui-researcher`/`frontend-architecture`'s solution-level concern). model-domain stays problem-level.
