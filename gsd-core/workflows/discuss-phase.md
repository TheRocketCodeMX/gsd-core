...
1b. **Initialize canonical refs accumulator** — Start building `<canonical_refs>` for CONTEXT.md. Sources:
   ...
   - **Project discovery artifacts (if present):** add `.planning/DOMAIN-MODEL.md` (subdomains + complexity), `.planning/adr/*.md` (architecture — rung + topology), `.planning/SECURITY-STRATEGY.md` (ASVS level, authz model, threat-model parent, security DoD), `.planning/FRONTEND-ARCHITECTURE.md` (FE structure/state + the FE side of the seam), `.planning/TEST-STRATEGY.md` (test levels + infra refs), `.planning/INFRA-STRATEGY.md` (compute/data/environments), and `.planning/CICD-STRATEGY.md` (pipeline/deploy/secrets) — when present. These are project-wide and apply to EVERY phase: planning and implementation MUST follow the domain model, the architecture decision (+ FE + security), and the test strategy; note each ref's role. When `TEST-STRATEGY.md` is present, the plan's test tasks must pull in the specific `gsd-core/references/<test-infra>.md` it links for the level being written.
