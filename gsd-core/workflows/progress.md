...
Then exit (do not fall through to the `init_incomplete` recovery below, to Route F, or to the missing-STATE suggestion). A mid-chain project — PROJECT.md carrying a `## Strategy Plan`, REQUIREMENTS.md present, ROADMAP.md/STATE.md not yet written — is exactly the shape `init_incomplete` (#4040) reports, so this check must run first; the roadmap is owed by `/gsd:roadmap`, not by a resumed `/gsd:new-project`.
<!-- FORK:strategy END -->
