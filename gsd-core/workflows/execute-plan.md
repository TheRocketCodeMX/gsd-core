...
- A deliverable a certification run drove end-to-end against a real environment → `kind: agentic_certification` with the run's evidence bundle as the `ref` (the transcript, plus screenshots/console captures where the driver was probed capable — never a test path), and `status` from the run's own verdict. Executors do not author these: `verify-work`'s certification step writes them back (see `verify-work/steps/agentic-certification.md`). Never file a certification claim you did not run.
