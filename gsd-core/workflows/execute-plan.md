...
- A task whose `<verify>` command passed or whose matching test passed → a `verification` entry with `kind` + `ref` (`tests/path#name`, Playwright screenshot ref, or command) + `status: pass`, and `human_judgment: false`. The one `kind` an executor never authors is `agentic_certification`: that entry belongs to a certification run against a real environment, its `ref` is the run's evidence bundle (transcript, plus screenshots/console captures where the driver was probed capable — never a test path), and `verify-work`'s certification step writes it back (`verify-work/steps/agentic-certification.md`). Never file a certification claim you did not run.
...
**Suite metrics (suite health).** If — and only if — this plan's own `<verification>` ran the project's **whole** test suite, record the `suite-metrics:` frontmatter block: `test_count` (the count the runner printed), `wall_clock_ms` (integer **milliseconds**, measured from a real clock bracket, not estimated, minimum 1 — the same unit the template, the post-merge gate, and `transition`'s compare all use; second-resolution rounding blinds T2 on sub-second suites), `containers_started` (from Testcontainers/docker output where visible, else `—`). If you ran only a subset, or ran nothing, **omit the block** — the phase's post-merge test gate records it instead (`execute-phase/steps/post-merge-gate.md`). Never estimate a number and never write `0` for "not measured": `transition`'s suite-health compare reads this block against the TEST-STRATEGY `## Suite health` baseline, and an invented row silently moves a T1–T4 trigger. `ms/test` is derived there, never recorded here.
