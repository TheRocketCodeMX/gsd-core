...
**Suite metrics (suite health).** Record the `suite-metrics:` frontmatter block only when this plan's own `<verification>` ran the project's **whole** test suite — `test_count`, `wall_clock_ms` (integer **milliseconds** from a real clock bracket, minimum 1 — the unit the template, the post-merge gate and `transition`'s compare share), `containers_started` (or `—`). Ran a subset, or nothing → **omit the block** (never estimate); the phase's post-merge test gate records it instead. Field rules: `<suite_metrics_guidance>` in `gsd-core/templates/summary.md`.
