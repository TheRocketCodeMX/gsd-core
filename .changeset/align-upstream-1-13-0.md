...
- **Workflows resolve `gsd_run` and prove which `gsd-tools` they run (#3831/#3848/#3809)** — the canonical launcher preamble proves the resolved `gsd-tools` is this package before running it and exports `GSD_IDENTITY_STATUS` (a handful of skills and fragments still carry the older `command -v gsd-tools` PATH arm without the identity proof — tracked, not yet closed).
