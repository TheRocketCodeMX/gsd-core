...
- **Do plans honor the brownfield build-loop contract? (Mode-gated)** Read `## Mode` — `gsd-tools query project mode`. When **Origin = rewrite-refactor** or **Code-quality = legacy/vibe-coded**, AND a task **changes behavior-bearing existing code**, the plan MUST follow `@~/.claude/gsd-core/references/brownfield-adaptation.md`'s build-loop contract: a **characterization-test task is sequenced BEFORE** the behavior-change task (the change task depends on it) and the change-region's disposition (follow/improve/refactor) is recorded. **BLOCKER** if a behavior-changing task on existing code has no preceding characterization/parity coverage (this is how "never silently change behavior" becomes un-skippable, not advisory). Greenfield / new-file work needs none — the gate fires only on the concrete trigger (changing existing behavior). If `LEGACY-INVENTORY.md` exists, also flag a capability it lists that no task covers (the "never lose a feature" gate).
</upstream_input>
