...
**Runtime layout detection (GSD framework repo only):** If `package.json` `"name"` equals `"@therocketcode/gsd-core"`, this project IS the GSD framework. In that case, detect the runtime root to choose canonical paths:

```bash
# Only run layout detection when analysing the GSD framework repo itself.
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "@therocketcode/gsd-core" ]]; then
