...
describe('#4153 regression: unresolved update targets stop before later workflow steps', () => {
  ...
  test('the unresolved path is explicit, ordered, and contains no mutation', () => {
    ...
    assert.match(step, /npx -y --package=@therocketcode\/gsd-core@latest -- gsd-core --global/);
    ...
    const mutationSpies = [
      ...
      { name: 'resolved installer', text: src, needle: 'npx -y --package=@therocketcode/gsd-core@"$TAG" -- gsd-core "$RUNTIME_FLAG"', after: end },
...
test('issue #815: install uses the selected tag, not a hardcoded @latest', () => {
  const robust = WF.match(/npx -y --package=@therocketcode\/gsd-core@"\$TAG" -- gsd-core/g) || [];
  ...
  assert.doesNotMatch(runUpdate, /--package=@therocketcode\/gsd-core@latest -- gsd-core/,
    'install lines must not hardcode @latest once --next exists');
  assert.doesNotMatch(runUpdate, /--package=@therocketcode\/gsd-core@(?:latest|next|beta|canary|rc) -- gsd-core/,
...
//   1. Cache-stale: bare `npx -y @therocketcode/gsd-core@<tag>` hits npx's local
//      cache and may pull an older version instead of the target tag.
//   2. Token-routing: Bash-tool wrappers misroute the `@` token in
//      `@therocketcode/gsd-core@<tag>`, causing npm to error with
//      "Unknown command: @therocketcode/gsd-core@<tag>".
//
// The robust form is:
//   npx -y --package=@therocketcode/gsd-core@"$TAG" -- gsd-core $ARGS
//
...
__t3130('bug #3130: update.md contains no bare npx invocations (cache-stale form)', () => {
  // Any occurrence of `npx -y @therocketcode/gsd-core@<something>` without `--package=`
  ...
  const stale = (src3130.match(/npx -y @therocketcode\/gsd-core@\S+[^\r\n]*/g) || []);
...
__t3130('bug #3130: update.md has exactly two robust resolved-install invocations', () => {
  ...
  const robust = (src3130.slice(start, end).match(/npx -y --package=@therocketcode\/gsd-core@\S+ -- gsd-core/g) || []);
