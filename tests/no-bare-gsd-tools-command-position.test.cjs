...
const PROSE_ALLOWLIST = [
  // FORK: upstream's three agent entries (gsd-executor.md:823, gsd-phase-researcher.md:33,
  // gsd-roadmapper.md:647) are deliberately absent. The fork's copies of those lines
  // spell the mention in the `gsd_run query …` resolver form, so there is no bare
  // `gsd-tools <verb>` to sanction there and the stale-entry guard below would flag them.
