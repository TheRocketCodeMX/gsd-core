...
// FORK:strategy BEGIN
    // Fork roadmap-after-strategy reorder (2026-07-14, guarded by
    // tests/roadmap-after-strategy.test.cjs): the roadmapper spawn no longer
    // lives in new-project.md — the shared gsd-roadmap skill owns the single
    // spawn point, and new-project dispatches to it. Upstream's #2376
    // tokenized-spawn assertions therefore apply to the DISPATCH seam here.
    assert.ok(/Skill\(skill="gsd-roadmap"|\/gsd:roadmap/.test(content),
      'new-project must dispatch roadmap creation to the gsd-roadmap skill (fork reorder)');
    assert.ok(!/subagent_type="gsd-roadmapper"/.test(content),
      'new-project must not spawn gsd-roadmapper inline (single spawn point is the gsd-roadmap skill)');
// FORK:strategy END
  });
...
// FORK:strategy BEGIN
    // Same fork roadmap-after-strategy adaptation as new-project.md above: the
    // roadmapper spawn (extend mode) lives in the gsd-roadmap skill.
    assert.ok(/Skill\(skill="gsd-roadmap"|\/gsd:roadmap/.test(content),
      'new-milestone must dispatch roadmap extension to the gsd-roadmap skill (fork reorder)');
    assert.ok(!/subagent_type="gsd-roadmapper"/.test(content),
      'new-milestone must not spawn gsd-roadmapper inline (single spawn point is the gsd-roadmap skill)');
// FORK:strategy END
  });
