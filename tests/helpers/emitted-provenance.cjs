...
const PROVENANCE_RULES = [
  // ── Verbatim engine payload ────────────────────────────────────────────────
  {
    ...
    // FORK:learn BEGIN
    // `visual` is the fork's vendored gsd-learn visual companion
    // (gsd-core/visual/*, zero-install pure-Node), emitted verbatim like the
    // other engine-payload subdirs; registered here exactly as an upstream
    // subdir would be so totality stays loud for any NEW family.
