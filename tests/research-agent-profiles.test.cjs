...
describe('bug #222 recurrence: orchestrator self-heals when synthesizer returns SUMMARY.md inline', () => {
  ...
  for (const wf of WORKFLOWS) {
    ...
    test(`${name} runs the #222 self-heal AFTER the synthesizer and BEFORE the roadmap step`, () => {
      // FORK: the roadmapper spawn moved out of new-project/new-milestone into the
      // shared gsd-roadmap skill (2026-07-14 roadmap-after-strategy reorder — see
      // tests/roadmap-after-strategy.test.cjs, which asserts the inline spawn is
      // GONE from both workflows). The ordering guard therefore anchors on the
      // roadmap hand-off (the `gsd-roadmap` skill dispatch / pointer) instead of an
      // inline `subagent_type="gsd-roadmapper"` spawn. Ported from the fork's
      // tests/bug-222-research-synthesizer-write-contract.test.cjs when upstream
      // consolidated that file into this suite (#1973).
      const text = fs.readFileSync(wf, 'utf8');
...
describe('gsd-project-researcher agent registration (#2419)', () => {
  ...
  test('new-project.md agent diagnostics are keyed on fields init actually emits (#21 P2a)', () => {
    // FORK (#21 P2a): init.new-project emits agents_installed / missing_agents /
    // agent_runtime / agents_dir — it never emitted required_agents_installed /
    // missing_required_agents / agent_skill_payloads_available. The doc used to
    // template a warning on those phantom keys, which could never render. Ported
    // from the fork's tests/bug-2419-project-researcher-agent.test.cjs when
    // upstream consolidated that file into this suite (#1976).
    const content = fs.readFileSync(NEW_PROJECT_PATH, 'utf-8');
    ...
    for (const phantom of ['required_agents_installed', 'missing_required_agents', 'agent_skill_payloads_available', 'agent_skill_payload_agents']) {
      assert.ok(!content.includes(phantom),
        `new-project.md must not template on the phantom init key ${phantom} (#21 P2a)`);
    }
  });
