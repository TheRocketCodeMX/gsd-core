...
const STUB_HOOKS = [
  ...
  'gsd-check-update.js',
  'gsd-config-reload.js',
  // FORK: the grounding-index refresh hook (#11) registers on FileChanged
  // alongside gsd-config-reload; without the stub the installer's existsSync
  // guard silently skips it and the 2-hook idempotency assertion below fails.
...
describe('enh-770: Claude install is idempotent for the new hook events', () => {
  ...
  test('re-running after persisted first install does not duplicate FileChanged hook', () => {
    ...
    // FORK: FileChanged legitimately carries 2 managed hooks since the fork's
    // grounding-index refresh (#11) registers alongside gsd-config-reload. The
    // idempotency contract is unchanged: exactly one entry per managed hook, no
    // dupes. Ported from the fork's tests/enh-770-claude-hook-events.test.cjs when
    // upstream consolidated that file into this suite (#1974).
