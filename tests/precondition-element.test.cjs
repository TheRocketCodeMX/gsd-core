...
describe('issue #1949: gsd-planner.md declares <precondition> emission', () => {
  ...
  // tests/agent-size-budget.test.cjs (upstream's own v1.9.0 body left 36 chars of
  // headroom under 49152 — no fork block fits; see docs/FORK-DELTA.md). Ratchet
  // back to 49152 if upstream shrinks.
  test('planner is under the 57344-char cap after adding <precondition> content', () => {
    const planner = read(PLANNER);
    assert.ok(
      planner.length < 57344,
      `gsd-planner.md is ${planner.length} chars, must be < 57344 (LF-normalized)`
    );
  });
});
...
describe('issue #1949: gsd-executor.md asserts <precondition> before task execution', () => {
  ...
  // FORK: cap mirrors the XL tier this fork records for gsd-executor in
  // tests/agent-size-budget.test.cjs (upstream's own v1.9.0 body left 494 chars of
  // headroom under 49152 — the fork's ~3.2K marked blocks cannot fit; see
  // docs/FORK-DELTA.md). Ratchet back to 49152 if upstream shrinks.
  test('executor is under the 57344-char cap after adding <precondition> content', () => {
    const exec = read(EXECUTOR);
    assert.ok(
      exec.length < 57344,
      `gsd-executor.md is ${exec.length} chars, must be < 57344 (LF-normalized)`
    );
  });
});
