 ...
 *
 * Regression tests for: https://github.com/TheRocketCodeMX/gsd-core/issues/1343
 */
...
describe('bug #685: Windows spawns must set windowsHide:true (no console-window flash)', () => {
  // FORK: this fork rewrote hooks/gsd-context-monitor.js as a calm knowledge-flush
  // nudge that NEVER spawns a child process (the upstream `record-session` spawn was
  // removed in fork 1.7.2 and never reinstated). The #685 concern — a console-window
  // flash from an unhidden spawn — is therefore satisfied vacuously, and the stronger
  // invariant is asserted instead: the hook must have no spawn at all. Ported here
  // when upstream consolidated tests/bug-685-windowshide-spawn.test.cjs into this
  // suite (#1975); the fork's version of that file carried the same exemption.
