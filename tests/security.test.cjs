...
describe('SECURE: ASVS level scaling (#1627)', () => {
  ...
  describe('gsd-planner.md security disposition', () => {
    ...
    // tests/agent-size-budget.test.cjs (upstream's own v1.9.0 body left 36 chars
    // of headroom under 49152 — no fork block fits; see docs/FORK-DELTA.md).
    // Ratchet back to 49152 if upstream shrinks.
    test('planner is under the 57344-char cap', () => {
      const content = fs.readFileSync(plannerPath, 'utf-8').replace(/\r\n/g, '\n').replace(/\r/g, '\n');
      assert.ok(
        content.length < 57344,
        `gsd-planner.md must be < 57344 chars (LF-normalized); got ${content.length}`
      );
    });
  });
