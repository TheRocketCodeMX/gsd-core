...
// FORK: cap aligned to the XL tier this fork records for gsd-planner in
// tests/agent-size-budget.test.cjs — upstream's own planner body (49,116 LF chars
// at v1.9.0) left 36 chars of headroom under 48K, so no fork block can fit under
// the original limit; see docs/FORK-DELTA.md. Ratchet back to 48K if upstream shrinks.
const PLANNER_EXTRACTED_LIMIT = 56 * 1024;  // 56K (XL) — proves extraction happened
