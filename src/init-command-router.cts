...
function routeInitCommand({ init, args, cwd, raw, error }: RouteInitCommandOptions): void {
  routeCjsCommandFamily({
    ...
    handlers: {
      ...
      'new-project': () => {
// FORK:strategy BEGIN
        // Fork adds --design/--no-design (provided-design detection, tests/init.test.cjs)
        // on top of upstream's --auto; flags ride in upstream's generic options record.
        const namedArgs = parseNamedArgsOrExit(args, { valueFlags: ['design'], booleanFlags: ['auto', 'no-design'], positionals: 2 }, error);
        init.cmdInitNewProject(cwd, raw, {
          auto: namedArgs['auto'],
          design: namedArgs['design'],
          noDesign: namedArgs['no-design'],
        });
// FORK:strategy END
      },
      'new-milestone': () => {
