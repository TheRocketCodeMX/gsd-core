...
function scanTodos(planDir: string): ScanOutcome<TodoItem> {
  ...
  try {
    ...
    return { items: [{ scan_error: true, filename: '', priority: '', severity: '', title: '', area: '', summary: '' }], acknowledged: 0 };
  }
  ...
  if (openFiles.length > 5) {
    results.push({ _remainder_count: openFiles.length - 5, filename: '', priority: '', severity: '', title: '', area: '', summary: '' });
  }
...
function scanUatGaps(planDir: string, cwd: string): ScanOutcome<UatGapItem> {
  ...
  for (const target of targets) {
    ...
    for (const file of scopeToPhase(
      files.filter(f => f.includes('-UAT') && f.endsWith('.md') && !f.includes('-UAT-superseded-')),
      target.dir,
    )) {
