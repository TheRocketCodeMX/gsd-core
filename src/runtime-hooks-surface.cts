...
/**
 * Project one legacy managed `node <script>` command string onto the
 * runtime-resolving runner, or return null when the entry must be left alone
 * (unparseable, arg-form, already-resolving shape, unmanaged script, or no
 * change needed).
 *
...
function projectLegacyManagedNodeCommand(
  ...
  if (CHAIN_RUNNER_COMMAND.test(trimmed) || RESOLVER_RUNNER_COMMAND.test(trimmed)) return null;
...
function rewriteLegacyManagedNodeHookCommands(settings: Settings, runnerToken: string, opts?: RewriteOpts): boolean {
  if (!settings || !runnerToken) return false;
  ...
  for (const entries of Object.values(settings.hooks || {})) {
    ...
    for (const entry of entries) {
      ...
      for (const h of entry.hooks) {
        const projected = projectLegacyManagedNodeCommand(h, runnerToken, platform, runtime);
  ...
  if (statusLine && typeof statusLine === 'object') {
    const projected = projectLegacyManagedNodeCommand(statusLine, runnerToken, platform, runtime);
    if (projected !== null) {
