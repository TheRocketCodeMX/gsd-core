...
**Process:**
1. Open a [Bug Report issue](https://github.com/TheRocketCodeMX/gsd-core/issues/new?template=bug_report.yml) — fill it out completely.
...
**Process:**
1. Open an [Enhancement issue](https://github.com/TheRocketCodeMX/gsd-core/issues/new?template=enhancement.yml) with the full proposal.  The issue template requires: the problem being solved, the concrete benefit, the scope of changes, and alternatives considered.
...
**Process:**
1. **Discuss first** — check [Discussions](https://github.com/TheRocketCodeMX/gsd-core/discussions) to see if the idea has been raised. If it has and was declined, don't open a new issue.
2. Open a [Feature Request issue](https://github.com/TheRocketCodeMX/gsd-core/issues/new?template=feature_request.yml) with the complete spec. The template requires: the solo-developer problem being solved, what is being added, full scope of affected files and systems, user stories, acceptance criteria, and assessment of maintenance burden.
...
# ... commit, push
gh pr create --base next --repo TheRocketCodeMX/gsd-core
```
...
`gh api repos/TheRocketCodeMX/gsd-core/branches/next/protection --jq '.required_status_checks.strict'`.
...
node scripts/release-notes/format-github-release-notes.cjs \
  --tag vX.Y.Z --repo TheRocketCodeMX/gsd-core --apply
```
...
If your PR adds, changes, deprecates, or removes user-visible behavior, you **must** update the relevant documentation in `docs/`. CI will fail any PR whose changeset fragment is typed `Added`, `Changed`, `Deprecated`, or `Removed` without also modifying at least one file under `docs/` ([#3213](https://github.com/TheRocketCodeMX/gsd-core/issues/3213)).
...
`.planning/` index into a commit whose message names one artifact — that is [#2269](https://github.com/TheRocketCodeMX/gsd-core/issues/2269),
