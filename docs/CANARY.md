...
npx @therocketcode/gsd-core@canary

# Pin to the canary dist-tag globally
npm install -g @therocketcode/gsd-core@canary

# Pin to an exact canary version
npm install -g @therocketcode/gsd-core@1.50.0-canary.1
```
...
npm install -g @therocketcode/gsd-core@next
```
...
File against the [issue tracker](https://github.com/TheRocketCodeMX/gsd-core/issues) with the `bug` template. Include the exact canary version (`gsd-core --version` reports it) so triage can route the report back into the `dev` stream rather than the stable stream.
...
- Stream architecture rationale: discussed across [#2727](https://github.com/TheRocketCodeMX/gsd-core/issues/2727), [#2773](https://github.com/TheRocketCodeMX/gsd-core/issues/2773) (codex schema-break and the resulting promotion bottleneck that motivated explicit stream isolation)
