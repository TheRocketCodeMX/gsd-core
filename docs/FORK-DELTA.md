...
<!-- SUPERSEDED (align-1.13.0): the fork's autonomous stop-banner step fragment (under autonomous/steps/) — upstream #3789 replaced the fixed-width box banners with Markdown headings, shrinking autonomous.md (38,279 B) far under the 40,960 cap; the fork's extraction was retired and the fragment (unreferenced) deleted. -->
- `gsd-core/references/autonomous-smart-discuss.md` (smart_discuss step body extracted from autonomous.md — #2196 char limit, the test's own named remedy)
...
| `scripts/lint-phase-enumeration-drift.cjs` | context | anchors-only | context-router capsule resolvers exempted from window-scoped enumeration (all-dirs by design) |
...
<!-- SUPERSEDED (align-1.13.0): the root bin/lib UI-safety-gate twin — upstream #3932 removed the hand-written root copy as dead code (installer + every workflow resolve gsd-core/bin/lib/ui-safety-gate.cjs only); the fork's negation-guard twin lives on in src/ui-safety-gate.cts (still registered). -->
...
| `.github/workflows/test.yml` | release | anchors-only | the scoped `windows` lane stands down when `full_matrix` is true (the two scoped steps gain `&& !(matrix.scope == 'windows' && needs.changes.outputs.full_matrix == 'true')`, plus an echo step naming why). On a change-everything PR — every upstream realignment — `windows_tests` degenerates to the whole Windows-relevant set (135 files) and cannot fit the scoped lane's default 600 s per-chunk budget on the Windows runners (all three shards chunk-killed on four consecutive heads of PR #85), while the `test-full` job, gated on the same `full_matrix`, already runs the entire unit suite on Windows and passes. Upstream never hits this (their PRs are scoped). Re-check at every pull whether upstream changed the scoped lane's budget or the `full_matrix` gating; drop when upstream makes the scoped Windows lane conditional itself |
...
| `gsd-core/workflows/fast.md` | fast-scope | markers (6) + anchors | PR #82 (contributed by @MidguetArturoGarciaTorres): the upstream `/gsd-fast` scope gate bounces on size ("≤ 3 file edits / ≤ 1 minute") and its commit step runs a blanket `git add -A` that sweeps unrelated in-flight changes into the task commit. Upstream text is unchanged on `open-gsd` main/next as of v1.13.0 (their #3585 deliberately kept `git add -A`), so the fork carries the fix: `scope_check` gates on uncertainty (research needed / ambiguous intent / dependency-pattern-schema change / >~10 files or multi-commit) and defaults to PROCEED; `execute_inline` snapshots pre-existing dirty paths and records touched paths; the `commit` step stages only `{touched paths}` and prints `git diff --cached --name-only` (in-fence — anchors); guardrails + success_criteria drop the size language. Regression test lives in `tests/planner-language-regression.test.cjs` (the `'fast.md': ['time_sizing']` ALLOWLIST entry was pruned). Superseded the day upstream adopts an uncertainty gate — record it then |
...
| `gsd-core/workflows/resume-project.md` | strategy | markers (1) | mid-strategy-chain route checked BEFORE upstream's #4040 `init_incomplete` recovery (same block as `progress.md`): a fresh project between `/gsd-new-project` and `/gsd-roadmap` has PROJECT.md + `## Strategy Plan` + REQUIREMENTS.md and no ROADMAP/STATE — exactly the shape #4040 classifies as an interrupted bootstrap. Without this route `/gsd-resume-work` would send the user back through `/gsd-new-project`; with it the answer is "continue the strategy chain or run `/gsd-roadmap`". Added in the v1.13.0 realignment (owner call on the wave report's item 1) |
...
| `gsd-core/workflows/plan-phase.md` | grounding, context | anchors-only + markers | SIZE-GATED (100280/<100344 bytes — v1.13.0 realignment: upstream's own body is 98,290 LF bytes, 10 under its 98,300 ADR-857 freeze and 14 under the 96 KiB XL tier cap, so BOTH constants were force-raised to measured+64 (tests/phase6-capstone-conformance.test.cjs, tests/workflow-size-budget.test.cjs); ratchet back at the next upstream shrink. History: 96875→91071 ratchet-down in v1.10.0, kept 94519 in v1.11.0); §1.6 elaboration gate, §13a grounding gate (+ upstream's #2770 fix kept), UI-hint sentence (now documenting upstream #2150), oracle files (anchors); §5.6 context freshness-gate directive + §10 `## Orchestrator curation` pre-checker directive (2 context marker pairs). The fork's §13a compression to `plan-phase-coverage-gate.md` is re-applied; the PRD-express trim is **upstream-absorbed** (their `steps/prd-express-path.md`) — the fork's stale `plan-phase/modes/prd-express.md` duplicate was PRUNED in the v1.11.0 realignment (upstream's reachability lint flagged it) |
...
<!-- SUPERSEDED (align-1.13.0): tests/emitted-drift-acks/*.json (45 fork fragments incl. align-upstream-1-10-0/1-11-0) and the emitted-drift-ack pre-merge lint script — upstream #3942 moved emitted-drift acknowledgments to `Emitted-Drift-Ack-Hash:` / `Emitted-Drift-Ack-Growth:` commit trailers and deleted the fragment directory, the sweep workflow and the lint; the fork follows (see docs/how-to/acknowledge-emitted-drift.md). Realignment ripples are acknowledged on the merge/fallout commits. -->
...
| `tests/skill-frontmatter-contract.test.cjs` | strategy | anchors-only | ported fork KNOWN_SKILLS entries (12 fork skills) after upstream deleted enh-2790 in consolidation #1970; v1.9.0: `FULL_BUDGET` 844→880 (upstream full.md = 829 lines, 15 headroom — the fork's 39 help-entry lines cannot fit); ratchet back on upstream shrink. v1.13.0: 880→890 (upstream full.md = 846 lines = its own ceiling; fork actual 885) |
| `tests/workflow-size-budget.test.cjs` | strategy | anchors-only | v1.9.0 realignment: `DISCUSS_PHASE_TARGET` 32000→34400 (upstream discuss-phase.md = 31,975 bytes, 25 headroom — the fork's ~2.3K marked blocks cannot fit); ratchet back on upstream shrink. v1.13.0: `XL_CAP` 98304→100344 for plan-phase.md (upstream body 98,290 — zero headroom; see the plan-phase.md row) |
| `tests/phase6-capstone-conformance.test.cjs` | context | anchors-only | v1.13.0 realignment: the ADR-857 `PRE_PHASE6` freeze for plan-phase.md raised 98300→100344 (measured 100280 + 64) — upstream's own body is 10 bytes under its freeze, so the fork's marked context/grounding blocks cannot fit at any compression; execute-phase.md keeps upstream's 93600. Ratchet back at the next upstream shrink (v1.10.0 execute-phase precedent) |
| `tests/agent-tracked-source-rule.test.cjs` | fidelity | anchors-only | v1.13.0 realignment: upstream #3645's "planner stays frozen" guard pins the agent under 49152 LF chars; the fork's planner carries FORK:fidelity blocks that predate #3645 and already lives in the XL tier `tests/emitted-sizes.test.cjs` records (same 57344 mirror as `tests/planner-decomposition.test.cjs` / `tests/precondition-element.test.cjs`). Ratchet back if upstream shrinks |
| `tests/failing-direction.test.cjs` | fidelity | anchors-only | v1.13.0 realignment: upstream #3172's row 30b "planner stays frozen" guard pins the agent under 49152 LF chars — same 57344 XL-tier mirror as `tests/agent-tracked-source-rule.test.cjs` (see that row) |
| `tests/artifacts.test.cjs` | strategy | anchors-only | the fork's e2e-5 F1 roster-vs-corpus drift gate (`CANONICAL_EXACT` vs every `.planning/<NAME>.md` the shipped corpus names) + its `NOT_ROOT_ARTIFACTS` exemption set; v1.13.0: `STATEX.md` exempted — upstream #3720's `pr-branch.md` names it as a NEGATIVE regex example, nothing writes it |
| `tests/ci-pr-mergeability.test.cjs` | release | anchors-only | upstream #3833's `NEVER_GATED` policy-lane list names `auto-close-unsolicited-prs.yml`, a workflow the fork does not ship (release plumbing removed at v1.9.0); the entry is dropped so the raw read does not ENOENT — the remaining policy lanes keep the never-gated contract |
| `tests/docs-hooks-table-parity.test.cjs` | grounding | anchors-only | `gsd-grounding-index-refresh.js` added to upstream's `EXPECTED_SURFACE_HOOKS` pin — the fork's FileChanged hook is registered on `src/runtime-hooks-surface.cts` and rostered in the docs hook tables |
| `tests/gsd-quick-batch-quick-regression.test.cjs` | release | anchors-only | row-48 (#3676) guard skips (reported) on an upstream-realignment branch — one whose changed set carries `.changeset/align-upstream-*.md` — because such a branch folds upstream's OWN quick.md / quick-batch edits plus the rebrand, which every scope heuristic in the test reads as #3676 phase work |
| `tests/no-bare-gsd-tools-command-position.test.cjs` | fidelity | anchors-only | upstream's three agent `PROSE_ALLOWLIST` line pins (gsd-executor.md:823, gsd-phase-researcher.md:33, gsd-roadmapper.md:647) removed — the fork's copies of those lines spell the mention in the `gsd_run query …` resolver form, so the stale-entry guard flags the pins as matching nothing |
| `tests/plan-phase-background-wait-wakeup.test.cjs` | context | anchors-only | v1.13.0 realignment: `PLAN_PHASE_PHASE6_LINE` mirror of the ADR-857 freeze raised 98300→100344 together with `tests/phase6-capstone-conformance.test.cjs` (see that row) |
| `docs/INVENTORY.md` | strategy | anchors-only | v1.13.0 realignment: upstream #3762's roster gate requires a hand-written row for every manifest entry — the fork's 12 commands, 11 workflows, 34 references, 8 CLI modules and 1 hook are rostered under fork-labelled subsections (Commands "Discovery, Strategy Chain & Knowledge (fork)", References "Strategy, Testing, Fidelity & Knowledge References (fork)") and in the shared Workflows / CLI Modules / Hooks tables |
...
| `tests/sync-skills-cross-runtime-refuse.test.cjs` | identity | anchors-only | v1.11.0 realignment: upstream test pins their installer command in the refusal message; the fork identity class rebrands npx hints to @therocketcode/gsd-core, so the expected string follows (workflow sync-skills.md:99/:138 is the source of truth) |
<!-- SUPERSEDED (align-1.13.0): tests/lint-allow-test-rule-refs.test.cjs — upstream #4060 moved the repo-baseline scan in-process (no spawnSync timeout to race), so the fork's 90 s child-timeout pin has nothing to pin. -->
| `src/runtime-artifact-conversion.cts` | skill-argument-hint | markers (4) | v1.11.0 matrix-B E1: upstream #2875 moved the LIVE skill converters here, orphaning the bin/install.js twin — argument-hint carry re-placed on the codex+cursor converters (the twin keeps its patched copies for its remaining direct call sites; lockstep pinned by tests/emitted-skill-argument-hint.test.cjs) |
| `src/runtime-artifact-conversion.cts` | fidelity | markers (1) | v1.11.0 matrix-B E3: brand-swap hardening (` Desktop` allow-list + `(?<!-)` lookbehind) re-placed on the LIVE neutralizeAgentReferences — the emitted-skill test probes the gsd-spike "non-Claude runtimes" corruption class |
...
| `bin/install.js` | uninstall-scoped-settings | anchors-only | `GSD_HOOK_LIB_FILES` is the UNION of upstream's roster and every helper actually shipped under `hooks/lib/` — upstream keeps shipping helpers without rostering them (#3582 `isolation-deny-reason.js` in v1.11.0; #4087 `cli-exit.js`/`exit-code-registry.js`/`hook-exit.js` and `git-probe.js` in v1.13.0), which leaves `hooks/lib` behind on uninstall; the hermetic update matrix (A9/B6) is the tripwire — re-diff `ls hooks/lib` against the roster at every pull. Also: uninstall step 6 loops over `settingsFilesToClean` (shared `settings.json` **plus** the scope-specific file from `hostBehaviors.settingsFileByScope`, i.e. `settings.local.json` on local Claude installs, #338). Without it, local uninstall leaves every managed hook registered against already-deleted scripts (realign 2.0.0 update-matrix B6) — was an UNMARKED delta and got dropped in the v1.9.0 merge; now manifest-pinned |
...
<!-- SUPERSEDED (align-1.11.0): tests/issue-607-legacy-cleanup.test.cjs folded upstream into the installer cluster (#3336/ad07f76a3, coverage continues in tests/install.test.cjs + tests/issue-607-installer-dry-run successors); the fork identity swaps ride the branding sweep in the fold destinations. -->
...
| `src/broken-windows.cts` | windows-ledger | anchors-only | `reason` column on `markFixed`, `amendWindow`, `reconcileLedger` (the single lenient read path) + review hardening: waived reasons cannot be emptied, duplicate ids rejected in the shared parse path (`WINDOWS_DUPLICATE_ID`), surplus positionals are a typed usage error, `repaired`/`normalized` computed independently, `stripHeaderProse` keeps a tool-version header change from rewriting true ledgers. v1.13.0: `writeLedgerAtomic` gained a `regenerateTable` opt-out of upstream #3689's table-drift guard, passed only by `windows reconcile` — the repair verb regenerates the table from the fenced JSON by contract. **upstream-PR candidate** |
...
| `src/shell-command-projection.cts` | grounding | anchors-only | `gsd-grounding-index-refresh.js` (fork #11 FileChanged hook) in BOTH managed-basename sets so uninstall and the legacy bare-`node` repair reach it. The former **managed-hook-coverage** entries (`gsd-agent-isolation-guard.js`, `gsd-write-guard.js`, plus `gsd-worktree-path-guard.js`) are **UPSTREAM-ABSORBED** (#3662, v1.13.0 realignment) — upstream now lists all three in both sets; the fork regression block in `tests/shell-command-projection-dispatch.test.cjs` stays as coverage |
...
<!-- SUPERSEDED (align-1.13.0): the src/config.cts bracket-guard — `cmdConfigSet` warned that `phase_id_convention: "bracket"` had no consumers, because bracket-form ROADMAP headings parsed to zero phases silently. Upstream #2761/#2867 (v1.13.0) now consume `bracket` on the read path (roadmap/validate/state/milestone complete), so the warning's premise is false and the row's own exit condition ("drop this block when the bracket consumers land upstream") is met. Guard, its test block, and both manifest entries removed; the migrator/write path is still upstream's pending work, not a silent failure. -->
...
| `src/uat.cts` | strategy | anchors-only | superseded-UAT archives (`-UAT-superseded-`) excluded from the `audit-uat` scan, and `parseUatItems`' result token widened to `[\w-]+` so `[pending-certifier]` is its own state, never plain `pending` — the archive restart creates to be inert was resurrected as phantom outstanding work, and the human was chased for the certifier's checkpoints (re-gate 6 N2). v1.13.0: upstream #3707 inverted `parseUatItems` from a `pending|skipped|blocked` whitelist to a PASS-set (every non-pass token surfaces), so the certifier skip that used to fall out of the whitelist is now stated explicitly (`UAT_CERTIFIER_RESULT`, skipped right after the PASS check). **UPSTREAM-ISSUE CANDIDATE** (the scan filter; the certifier token is fork doctrine) |
...
<!-- SUPERSEDED (align-1.13.0): the tests/config.test.cjs bracket-guard contract block — removed together with the src/config.cts guard it pinned (see the note above). -->
