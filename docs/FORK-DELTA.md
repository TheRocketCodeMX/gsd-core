...
| `gsd-core/workflows/verify-work.md` | strategy | anchors-only | certification in the loop (testing-certification design spec §5, §7): the `agentic-certification` section gate before UAT (placed between `extract_tests` and `create_uat_file` so `present[]` exists and no checkpoint has been shown; dispatches the fork-owned `verify-work/steps/agentic-certification.md`), the `source: agentic` / outcome-line write block inside `create_uat_file`, and the `coverage_gap_capture` step between `diagnose_issues` and `plan_gap_closure` — the "which fast test was missing" question that gives `TEST-STRATEGY.md` its second (append-only) writer |
...
| `gsd-core/workflows/execute-phase/steps/post-merge-gate.md` | strategy | anchors-only | suite-metrics capture (design spec §8.3): `SUITE_START_EPOCH` / `SUITE_WALL_CLOCK_SEC` around the test run the gate already performs, plus **Step C**, which records `{test_count, wall_clock, containers_started}` into the wave's SUMMARY frontmatter. This gate is the only place in the loop that runs the whole suite, so it is where the measurement is taken; a timed-out or absent runner records nothing rather than a zero |
