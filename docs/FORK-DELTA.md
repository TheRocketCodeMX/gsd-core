...
<!-- SUPERSEDED (align-1.13.0): gsd-core/workflows/autonomous/steps/stopped-banner.md — upstream #3789 replaced the fixed-width box banners with Markdown headings, shrinking autonomous.md (38,279 B) far under the 40,960 cap; the fork's extraction was retired and the fragment (unreferenced) deleted. -->
...
<!-- SUPERSEDED (align-1.13.0): bin/lib/ui-safety-gate.cjs — upstream #3932 removed the hand-written root copy as dead code (installer + every workflow resolve gsd-core/bin/lib/ui-safety-gate.cjs only); the fork's negation-guard twin lives on in src/ui-safety-gate.cts (still registered). -->
