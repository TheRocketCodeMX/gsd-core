...
- New surfaces should land here first, then propagate to the broad docs. The drift-control test in `tests/inventory-manifest-sync.test.cjs` anchors the roster contents against the filesystem.
...
| `tool-provenance.md` | Fork law (#74): every planning artifact — phase dirs, state files, completion markers — is created through `gsd-tools` commands, never hand-made; the formula is canonical and the gates only fire through the command. |
...
| `broken-windows.cjs` | Broken-windows ledger library (issue #1950) — typed IR + I/O for `.planning/WINDOWS.md` (cross-phase defect register); pure `parseLedger`/`renderLedger`/`appendWindow`/`markWaived`/`markFixed`/`amendWindow`/`reconcileLedger`/`openCount` + I/O `cmdWindowsStatus`/`cmdWindowsAppend`/`cmdWindowsWaive`/`cmdWindowsMarkFixed`/`cmdWindowsAmend`/`cmdWindowsReconcile`; frozen `REASON` enum for typed-error assertions; CLI surface `gsd-tools windows status\|append\|waive\|fixed\|amend\|reconcile`. Generated from `src/broken-windows.cts` |
