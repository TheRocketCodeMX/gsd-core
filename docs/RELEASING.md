...
`scripts/sync-upstream.sh` rewrites all six coordinate forms (`@therocketcode/gsd-core`, its regex-escaped variant, `TheRocketCodeMX/gsd-core` + escaped, the `therocketcode-gsd-core` cache slug, and the `%40therocketcode%2Fgsd-core` npm badge). Legacy `@opengsd/get-shit-done-*` / `@opengsd/gsd-sdk` migration refs are deliberately left intact. <!-- gsd-allow-legacy-name -->
