...
**Sources are FILES — so DIRECT exploration is enforceable, not just oracle-diffing.** The design arrives as *exported artifacts* (Stitch / Claude-design / Figma-Make → HTML/CSS/component files, per `design-ingestion.md:16`), and vibe/refactor/additional apps are code. A Read-only agent can read a file, so the "gate can't see the raw source" caveat (written for un-fetchable Figma) mostly dissolves. Two tiers, both file-grounded:
- **Upstream — explore the actual source files → produce a faithful, cross-checked inventory.** The 3 researchers + 2 mappers read the real files. **Gap: the 2 mappers have NO source-precedence hook** — add it (their fidelity is upstream of everything). The inventory is *derived from the files and cross-checked against them*, not trusted blind.
- **Downstream — cite the actual source, verified against the file.** The `## Grounding` block carries **two citation kinds**: *strategy citations* (the ADR rung etc. — the distilled decision) AND **source-direct citations** — a specific fact from the actual source file with `file:location` (e.g. `design/checkout.export.html:142 → single address input`; `legacy/src/orders.py:88 → status-transition guard`), which the gate greps the **real file** to verify. You cannot write it without opening the file — this forces exploration of the *source itself*, not the abstraction.
- **The oracle (`DESIGN-INVENTORY`/`LEGACY-INVENTORY`) becomes a cross-checked index, not a crutch** — verified faithful to the files it summarizes. (The current gate's "never the raw design", `gsd-verifier.md:461`, was premised on un-fetchable Figma; for file-based exports the Read-only gate CAN read the file, so we extend it to verify against the file too.)
- **Residual limit (small):** (a) a truly un-fetchable live source (auth-gated prototype URL, no export) falls back to oracle-only — rare here; (b) *thoroughness* is bounded, mitigated by **coverage-style citations** (enumerate every screen/endpoint from the actual file, like the LEGACY-INVENTORY matrix). But "did it read the real file, and does grounding match it" is now **enforced**.
