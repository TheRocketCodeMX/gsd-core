...
**FALLBACK — IMPLEMENTED (2026-07-18):** the first E2E run confirmed the soft delivery (no host branch acts on contribution text), and the follow-up investigation proved no legal capability shape (step/gate/contribution) reaches the orchestrator's own execution flow. The recorded remediation shipped with a user-approved ceiling raise (94519→94900, #1298 precedent): two marked `FORK:context` directive lines in plan-phase.md — the §5.6 **Context freshness gate** (run the fragment's `context verify` bash pre-planner) and the §10 **Curation first** (append `## Orchestrator curation (<date>)` pre-checker). The transcript-grep asserts above stay as written and are now satisfiable; ratchet the ceiling back down at the next upstream shrink.
