...
```bash
npx @therocketcode/gsd-core --kimi-code --global
```
...
```bash
npx @therocketcode/gsd-core --kimi-code --global --reclaim-kimi-legacy
```
...
- **Do I need to uninstall the old `--kimi` install first?** No — `--kimi-code --global` writes to `~/.kimi-code/`, which is separate. But if you no longer use Python kimi-cli, uninstalling the old install keeps things clean: `npx @therocketcode/gsd-core --kimi --global --uninstall`.
