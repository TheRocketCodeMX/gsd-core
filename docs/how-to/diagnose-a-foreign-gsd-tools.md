...
[#3129](https://github.com/TheRocketCodeMX/gsd-core/issues/3129) is the worked example.
...
prove it is `@therocketcode/gsd-core`.

If you got here from that warning, it looks like this:

```text
WARNING: "/some/path/gsd-tools.cjs" did not prove it is @therocketcode/gsd-core - it is either a
different package or an @therocketcode/gsd-core older than the runtime-identity verb.
```
...
Re-run `gsd-tools runtime-identity` and confirm it reports `@therocketcode/gsd-core`.

## It is an old gsd-core

The resolved path is inside `@therocketcode/gsd-core`, but `runtime-identity` is not a known
command. That version predates the verb. Nothing is wrong beyond being out of date:

```bash
npm install -g @therocketcode/gsd-core@latest
```
...
binary ([#381](https://github.com/TheRocketCodeMX/gsd-core/issues/381)). Upgrade:
...
- `ok` — the resolved tool proved it is `@therocketcode/gsd-core`. Nothing to do.
...
- [#3129](https://github.com/TheRocketCodeMX/gsd-core/issues/3129) — the incident this prevents
