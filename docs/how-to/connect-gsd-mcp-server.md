...
```jsonc
{
  "gsd": {
    "command": "npx",
    "args": ["-y", "@therocketcode/gsd-core", "gsd-mcp-server"],
...
```jsonc
{
  "mcp": {
    "gsd": {
      "type": "local",
      "command": ["npx", "-y", "@therocketcode/gsd-core", "gsd-mcp-server"],
      "timeout": 10000
    }
  }
}
...
- **`command not found: gsd-mcp-server`** — invoke via `npx` as shown above, or
  install the package globally first (`npm i -g @therocketcode/gsd-core`).
...
- **The host lists no GSD tools** — confirm the server starts in isolation:
  `npx @therocketcode/gsd-core gsd-mcp-server` then send an `initialize` request on
