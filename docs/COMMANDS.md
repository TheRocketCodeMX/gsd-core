...
[#3309](https://github.com/TheRocketCodeMX/gsd-core/issues/3309)). The same migration
...
| `model_reasoning_effort` with no `model` | the orphaned effort line is removed ([#838](https://github.com/TheRocketCodeMX/gsd-core/issues/838)) |
...
| `orphaned_reasoning_effort` | A `model_reasoning_effort` with no `model`, leaving the model following your Codex session while the effort follows GSD ([#838](https://github.com/TheRocketCodeMX/gsd-core/issues/838)) |
...
| Field | Type | Value |
|---|---|---|
| `packageName` | string | Always `@therocketcode/gsd-core`. Baked at build time from `package.json`, so it survives an installed tree that carries no real `package.json`. |
...
"@therocketcode/gsd-core"}` — and exports the outcome as `GSD_IDENTITY_STATUS`:

| `GSD_IDENTITY_STATUS` | Meaning |
|---|---|
| `ok` | The resolved tool proved it is `@therocketcode/gsd-core`. |
| `unverified` | It did not. Either a different package, or an `@therocketcode/gsd-core` older than this verb. The preamble prints one line to stderr and continues — the rollout is warn-then-fail. |
